...
bool cmp_data(const data_t* d1, const data_t* d2, CMP compare); //function that compares two structs
...
int main(void){
    int i = 0, j;
    queue_t queue = {3, 0, NULL, NULL, greater}; //queue init
    data_t s[5] = {
        {1, "customer 1"},
        {2, "customer 2"},
        {3, "customer 3"},
...
bool cmp_data(const data_t* d1, const data_t* d2, CMP compare){
    bool expression = d1->number < d2->number;
    return (compare == less? expression : !expression);
}
    ...
    if(queue_full(queue)){ //if queue is full, we can`t push an element to it`s end
        ...
        if((temp = (queue_data_t*) malloc(sizeof(queue_data_t))) == NULL){
            ...
            if(queue->head == NULL){ //if head is equal to NULL val then the element to be enqueued is our new head
                ...
                pt = queue->head;
                if(!cmp_data(&pt->data, &temp->data, queue->compare)){
                    temp->next = pt;
                    queue->head = temp;
                }
                else{
                    while(pt->next != NULL && cmp_data(&pt->next->data, &temp->data, queue->compare)){
                        pt = pt->next;
                    }
                    temp->next = pt->next;
                    pt->next = temp;
